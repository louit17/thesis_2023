...
import pandas as pd
df = pd.DataFrame(columns=['patient_id', 'polyp_number', 'label_number',  "min_value_contour", "max_value_contour", "mean_value_contour", "contour_pixels_depth"])



def create_segmentation_area_mask(dims, name_of_img):
    ...
    return boolean_mask_segmentation
...
def run(input_path, output_path, model_path, model_type="large"):
    ...
    for ind, img_name in enumerate(img_names):
        ...
        boolean_mask = create_segmentation_area_mask(img.shape[:2], img_name)
...
df.to_csv("depth_for_segmentation_area.csv",sep=';')