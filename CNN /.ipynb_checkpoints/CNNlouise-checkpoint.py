...
def rescale_data(img):
    """ Rescale images to have float channels \in [0, 1], boolean masks.
        ...
        TODO: this is hard coded and could be automated based on the input format and range.
    """
    img = img / 255
...
def augment_data(img, label):
    """ Data augmentation by rotation and flipping.
        ...
        TODO: This is rather messy and inefficient right now, just straight forward for testing. Clean this up later. :-)
              Actually, the whole data loading process doesn't seem to be optimised.
              https://www.tensorflow.org/api_docs/python/tf/image
              https://www.tensorflow.org/tutorials/images/data_augmentation

        TODO: we could rotate images by random angles, as we would only loose a bit of the circular shape.
              This might actually help to decrease border effects, as the border to the black background is
              then shifting slightly. But might also make things worse for the same reason. :-)
    ...
    for i in range(1, 4):
        ...
        #masks.append(tf.image.rot90(mask, i))
        images.append(tf.image.rot90(flipped_img, i))
        #masks.append(tf.image.rot90(flipped_mask, i))
...
early_stop = tf.keras.callbacks.EarlyStopping(monitor = 'val_accuracy', patience = 10, min_delta = 0.001, restore_best_weights = True)
